...
This document describes an Operator based deployment for MCP Gateway instances. It builds heavily on the design described in [isolated gateway deployment](./isolated-gateway-deployment.md) and it is recommended to read that document first.


## Use Cases

- Allow OLM based installation and upgrade 
- Automate the setup of MCP Gateway Instance(s) and configuration
...
- Simplifying the setup and configuration of the MCP Gateway instance
...
Not directly captured in detail but required is the packaging of the current controller into the correct OLM format and metadata (CatalogSource, Package Bundle etc). As the project is in a compatible kubebuilder structure and OLM uses kubebuilder, this packaging should not cause too many issues.
...
**Reconcile of a valid MCPGatewayExtension:**
...
Some of these flags will also be able to be set via the `MCPGatewayExtension` resource if needed:
